...
import { CONTENT, SEO_KEYWORDS_LIST } from '../constants';
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans text-slate-800 bg-sand-100">
      ...
      >
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="hidden md:flex items-center gap-8">
              <div className={`flex gap-6 text-sm font-medium ${isScrolled ? 'text-slate-600' : 'text-white/90'}`}>
                <Link to="/" className="hover:text-brand-500 transition-colors">{t.nav.home}</Link>
                <a href="#amenities" className="hover:text-brand-500 transition-colors">{t.nav.amenities}</a>
                <a href="#location" className="hover:text-brand-500 transition-colors">{t.nav.location}</a>
                <Link to="/blog" className="hover:text-brand-500 transition-colors">{t.nav.blog}</Link>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <a 
                  ...
                  className="bg-brand-500 hover:bg-brand-600 text-white px-5 py-2.5 rounded-full font-semibold text-sm transition-all shadow-lg hover:shadow-brand-500/30"
                >
                  {t.nav.bookNow}
                </a>
              </div>
        ...
        {/* Mobile Nav Overlay */}
        {isMobileMenuOpen && (
          <div className="absolute top-full left-0 w-full bg-white shadow-xl border-t p-4 flex flex-col gap-4 md:hidden animate-in slide-in-from-top-5">
            <Link to="/" className="text-lg font-medium text-slate-800">{t.nav.home}</Link>
            <a href="#amenities" className="text-lg font-medium text-slate-800">{t.nav.amenities}</a>
            <a href="#location" className="text-lg font-medium text-slate-800">{t.nav.location}</a>
            <Link to="/blog" className="text-lg font-medium text-slate-800">{t.nav.blog}</Link>
            <a 
              href="https://www.airbnb.com/rooms/1409650618945019706" 
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
            <div className="col-span-1 md:col-span-2">
              ...
              <div className="flex gap-4">
                <a href="#" className="p-2 bg-white/10 rounded-full hover:bg-brand-500 transition-colors"><Instagram size={20} /></a>
                <a href="#" className="p-2 bg-white/10 rounded-full hover:bg-brand-500 transition-colors"><Mail size={20} /></a>
              </div>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-2 text-brand-100 text-sm">
                ...
                <li className="pt-2"><Link to="/location" className="text-brand-400 hover:text-white transition-colors">View on Map →</Link></li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-3 text-brand-100 text-sm">
                <li className="flex items-center gap-2"><Phone size={16} /> +507 0000-0000</li>
          ...
          <div className="border-t border-white/10 pt-8 mb-8">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-2">
              {SEO_KEYWORDS_LIST.map((keyword, index) => (
                <span key={index} className="text-xs text-brand-200/60 hover:text-brand-100 cursor-pointer transition-colors">
          ...
          <div className="pt-8 border-t border-white/10 text-center text-brand-200 text-sm flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="text-xs opacity-50">
              Vacation Rental Website by Maria Chiquita Rentals
            </div>
          </div>
        </div>
      </footer>
